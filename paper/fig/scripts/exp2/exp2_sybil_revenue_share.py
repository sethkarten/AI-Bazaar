...
Mean ± 1σ bands across seeds. Majority threshold at 0.5.
...
def get_revenue_share_series(run_dir: str):
    """Return (ts_array, values_array) or None."""
...
def main():
    ap = argparse.ArgumentParser(description="Exp2 Fig E: Deceptive Revenue Share")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as ex:
            ...
            for future in concurrent.futures.as_completed(future_map):
                ...
                print(f"  [{done}/{total}] K={k} rep={int(rv)} seed={seed}"
                      f" — {'ok' if data is not None else 'empty'}", flush=True)
    ...
    fig.suptitle("Sybil Revenue Share over Time", fontsize=10, fontweight="bold")
    ...
    for k in K_ALL:
        ...
        for rv in [True, False]:
            ...
            lbl = f"K={k} ({sat:.0%} sybil), {rep_tag}"
            plot_band(ax, entry, color, lbl, ls=LS_REP[rv])

    ax.set_xlabel("Timestep")
    ax.set_ylabel("Sybil revenue share (fraction of total)")
    ax.set_ylim(0, 1.05)
