...
| **3. Pricing** | Each firm sets prices and posts **quotes** to the market (one quote per good they have in stock). Firms may use last step’s prices as context. With `--info-asymmetry`, firms see only a noisy summary of competitor prices (bazaar_env). |
...
| **7. Cleanup & Overhead** | The market's quotes and order queue are cleared. Each firm (still in business) pays a fixed overhead cost (50 per step). Money is debited from the firm; firms can go "out of business" if they cannot pay. (Sales/tracking updates happen in phase 6.) | firm’s tracking (e.g. `total_quantity_sold_by_good`). The market’s quotes and order queue are cleared for the next timestep. |
| **8. Platform fee** | Each firm (still in business) pays a fee equal to 5% of current cash. Money is debited and leaves the system (no platform agent). Firms can go “out of business” if they cannot pay. |
