...
## 15. Logged State and Metrics (Lemon Scenario)

This section summarizes the **per-timestep state fields** and **auxiliary log files** that are
specific to, or especially important for, the Lemon Market scenario. These are what the
dashboard and figure scripts consume.

### 15.1 Per-timestep state (`state_t*.json`)

Each call to `BazaarWorld.save_state()` writes a JSON snapshot
`logs/<run_name>/state_t{timestep}.json` containing:

- **Global fields**
  - `timestep`: integer timestep index `t`.
  - `ledger.money`: mapping `agent_id -> cash` for all firms and consumers.
  - `ledger.inventories`: mapping `agent_id -> {good -> quantity}` (primarily `"car"`).
  - `total_fees`: platform fees charged this step (10% of firm cash, see code).
  - `filled_orders_count`: total number of filled buyer orders this step (volume `V_t`).
  - `filled_orders_count_by_firm`: `{firm_id -> count}` of filled orders this step.
  - `views_by_firm`: (non-Lemon specific) `{firm_id -> views}` for discovery charts.

- **Lemon-specific aggregate metrics**
  - `lemon_market_listings_count`: number of active listings posted to the market this step
    after merging new and unsold listings.
  - `lemon_market_bids_count`: number of buyer bids submitted this step.
  - `lemon_market_passes_count`: number of buyers that **passed** (no bid) this step.

- **Lemon listing logs**
  - `lemon_market_new_listings`: list of dicts, each new listing created this step:
    - Keys: `id`, `firm_id`, `description`, `price`, `reputation` (at listing time),
      `quality` (tier label), `quality_value` (float), `timestep_posted = t`.
  - `lemon_market_unsold_listings`: list of unsold `Listing` dicts carried over to the next
    step (same fields as the `Listing` dataclass in `market_core`).

- **Per-firm state (`firms` array)**
  - `name`, `in_business`, `persona`, `sybil` (bool flag for Sybil identities).
  - `cash`, `profit`, `reputation`.
  - `prices`: last posted prices (empty in Lemon, where prices live on listings).
  - `inventory`: e.g. `{ "car": quantity }`.
  - `expenses_info`: rolled-up expenses:
    - `supply_cost`, `overhead_costs`, `taxes_paid`, `platform_fees`.
    - `supply_by_good`: list of `{good, quantity, unit_cost, total_cost}` entries.
  - `sales_info`: list of per-sale dicts:
    - Keys: `good`, `quantity_sold`, `requested_quantity`, `price`,
      and for Lemon sales, `quality_value`.
  - `sales_by_good`: cumulative quantity sold per good.
  - `sales_this_step`: quantity sold per good in the current step.
  - `views_this_step`, `conversion_rate`: derived from `views_by_firm` and `sales_info`.
  - `listings_posted_this_step`: count of listings the firm posted this step.
  - `quality_sold_this_step`: list of `quality_value` for items sold this step.

- **Per-consumer (buyer) state (`consumers` array)**
  - `name`, `labor`, `income`, `cash`.
  - `utility`, `goods_utility`, `cash_utility`, `labor_disutility`.
  - `inventory`: e.g. `{ "car": 1 }` for buyers who purchased a car.
  - `willingness_to_pay`: (unused in Lemon when buyers are `BuyerAgent`, but preserved
    for consistency).
  - `eWTP`: per-good expected WTP; used for cross-environment dashboards.
  - `consumer_surplus`: **per-step** surplus for this consumer in this timestep:
    `CS_t = q_t · V_MAX - price_paid` in Lemon (Section 14.4).
  - `diary`: last diary entry (typically empty in Lemon with `--no-diaries`).
  - `discovery_this_step`: for THE_CRASH; kept for schema compatibility.

### 15.2 Run-level metadata files

All runs (including Lemon) also write the following JSON files to
`logs/<run_name>/`:

- **`experiment_args.json`**
  - Full `argparse` configuration used for the run (`vars(args)`), plus:
    - `rerun_command`: a copy-paste-able `python -m ai_bazaar.main ...` command
      reconstructing the run.
  - Key Lemon-related arguments: `consumer_scenario`, `num_firms`, `num_consumers`,
    `sybil_cluster_size`, `reputation_alpha`, `reputation_initial`, `sybil_rho_min`,
    `discovery_limit_consumers`, `no_buyer_rep`, `llm`, `max_tokens`, etc.

- **`consumer_attributes.json`**
  - One entry per consumer/buyer with stable attributes at initialization:
    - `name`, `ces_params`, `c`, `sigma`, `delta`, `llm_model`, `skill`,
      `risk_aversion`, `epsilon`, `beta`, `goods`.
  - In Lemon, buyers are `BuyerAgent` instances; many CES-specific fields may be `null`,
    but the file is still written for consistency and dashboard tooling.

- **`firm_attributes.json`**
  - One entry per firm/seller:
    - `name`, `goods`, `supply_unit_costs`, `persona`, `system_prompt`.
  - Useful for:
    - Distinguishing honest vs. Sybil identities (via `persona` and `sybil` flags in state).
    - Inspecting the exact system prompts used for seller agents.

- **Optional alignment traces and token usage**
  - `alignment_traces.jsonl` (if `--log-alignment-traces` is enabled):
    - Per-timestep `(state, firms, outcome)` traces used for SFT (primarily for Crash,
      but Lemon traces can also be collected).
  - `<run_name>_token_usage.json`:
    - Written by `main.run_marketplace_simulation` into the same `logs/<run_name>/`
      folder, aggregating **all** firm and buyer agents:
      - `input_tokens`, `output_tokens`, `requests`.
    - The Streamlit dashboard’s **Token Usage** tab reads these files to show LLM cost
      for each run.